...
namespace szedarserver.Infrastructure.Services
{
    public class DoubleEliminationService : IDoubleEliminationService
    {
        ...
        public DoubleEliminationService(ITournamentService tournamentService, IMapper mapper,
        ...
        public async Task AddResultAsync(Guid matchId, MatchDTO result)
        {
            ...
            if (match.Result.Count() != 2
                || (nextMatch != null && nextMatch.Result != null &&
                    nextMatch.Result.SingleOrDefault(r => r.Win) != null)
                || (nextLoserMatch != null && nextLoserMatch.Result != null &&
                    nextLoserMatch.Result.SingleOrDefault(r => r.Win) != null))
            {
            ...
            if (nextLoserMatch != null && nextLoserMatch.Result != null &&
            ...
            if (match.Round <= 2)
            {
                await _tournamentRepository.AddResultsAsync(CheckLoserMatches(nextLoserMatch, allMatches,
                    loser.PlayerId, results));
            }
            else
            {
        ...
        private List<Result> CheckLoserMatches(Match baseMatch, List<Match> matches, Guid playerId,
            List<Result> results)
        {
            ...
            if (match != null)
            {
                if (match.Result.First().Win)
                {
                    ...
                    return CheckLoserMatches(matches.Single(m => m.MatchCode == baseMatch.NextMachCode), matches,
                        playerId, results);
                }
